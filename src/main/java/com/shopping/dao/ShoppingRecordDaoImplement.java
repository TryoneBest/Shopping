...
public class ShoppingRecordDaoImplement implements ShoppingRecordDao {
    ...
    public boolean updateShoppingRecord(ShoppingRecord shoppingRecord) {
        ...
        String sql = "update shopping_record set order_status=" + shoppingRecord.getOrderStatus()
                + " where user_id=" + shoppingRecord.getUserId()
                + " and product_id=" + shoppingRecord.getProductId()
                + " and time='" + shoppingRecord.getTime() + "'";
        //        Query query = sessionFactory.getCurrentSession().createQuery(hql);
